...
| `PUT`    | `/api/library-admin/listings/{listingId}/digital-asset` | `LibraryOwner` | Replace an owned digital listing's PDF for future purchases.  |
...
| `POST`   | `/api/notifications/device-token` | Authenticated  | Deprecated compatibility alias for device-token registration.  |
...
The mobile client should call `PUT /api/notifications/devices` with `{ "deviceToken": "..." }` after login and whenever Firebase rotates the token. The older `POST /api/notifications/device-token` route accepts the same body and writes to the same `PushDevices` store, but new clients should use `PUT`. The client should call the matching `DELETE` route on logout when possible. Approval pushes contain `type=library_registration_approved`, the `libraryId`, `approvalStatus=Approved`, and `requiresReauthentication=true`; role promotion invalidates the old login family, so the app should take the user to a fresh sign-in instead of treating the old access token as authorized for library-owner screens. Listing publication and digital-asset-update pushes are written to `ListingPushNotifications` in the same transaction as the listing change and sent by a retrying background worker, so Firebase latency never blocks the HTTP response. A single publication uses `type=NEW_LIBRARY_BOOK`; multiple publications committed together for one library use one `type=NEW_LIBRARY_BOOK_BATCH` push with `libraryId` and `bookCount` instead of one push per book.
